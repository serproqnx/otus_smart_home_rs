...
pub struct Room {
  pub name: &'static str,
  pub devices: HashMap<&'static str, Box<dyn SmartHomeUnit + Send>>,
}
...
impl Room {
  ...
  pub fn del_device(&mut self, name: &'static str) -> Option<Box<dyn SmartHomeUnit + Send>> {
    self.devices.remove(name)
  }

  pub fn add_device(&mut self, device: Box<dyn SmartHomeUnit + Send>) {
  ...
  pub fn get_devices_list(&self) -> &HashMap<&str, Box<dyn SmartHomeUnit + Send>> {
